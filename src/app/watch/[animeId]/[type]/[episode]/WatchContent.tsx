...
export default function WatchContent({
  ...
  const fetchEpisode = useCallback(async () => {
    ...
    try {
      // Fetch datasets across providers
      const [reanimeData, anikotoData] = await Promise.allSettled([
        getWatch("reanime", Number(animeId), type, `reanime-${episode}`),
        getWatch("anikoto", Number(animeId), type, `anikoto-${episode}`)
      ]);

      let consolidatedServers: any[] = [];
      let downloadsPayload: any = null;

      // Extract reanime stream maps
      if (reanimeData.status === 'fulfilled' && reanimeData.value?.allServers) {
        consolidatedServers = [...reanimeData.value.allServers];
        downloadsPayload = reanimeData.value.downloadUrl;
      }

      // Extract and map anikoto stream structure cleanly
      if (anikotoData.status === 'fulfilled' && anikotoData.value) {
        const targetPayload = type === 'dub' ? anikotoData.value?.sdub : anikotoData.value?.ssub;
        if (targetPayload?.streams) {
          const mappedAnikotoServers = targetPayload.streams.map((s: any) => ({
            name: `${s.server} (Anikoto)`,
            embed: s.url,
            type: s.type,
            isDefault: s.default || false
          }));
          consolidatedServers = [...consolidatedServers, ...mappedAnikotoServers];
        }
        if (!downloadsPayload && anikotoData.value?.downloadUrl) {
          downloadsPayload = anikotoData.value.downloadUrl;
        }
      }

      const compositeEpisodePayload = {
        title: `Episode ${episode}`,
        allServers: consolidatedServers,
        downloadUrl: downloadsPayload
      };

      setEpisodeData(compositeEpisodePayload);
      
      if (compositeEpisodePayload.allServers.length) {
        const defaultServer =
          compositeEpisodePayload.allServers.find((s: any) => s.name === "HD-2" || s.isDefault) ||
          compositeEpisodePayload.allServers[0];
  ...
  const fetchAnimeDataFrom = useCallback(async () => {
    try {
      ...
      // Unpack mapping layers from reanime
      const providerEpisodesReanime =
        type === "dub"
          ? data?.reanime?.episodes?.dub || data?.reanime?.dub
          : data?.reanime?.episodes?.sub || data?.reanime?.sub;

      // Unpack mapping layers from anikoto
      const providerEpisodesAnikoto = 
        type === "dub"
          ? data?.anikoto?.episodes?.dub || data?.anikoto?.dub
          : data?.anikoto?.episodes?.sub || data?.anikoto?.sub;

      // Unify dynamic list tracking prioritising the most verbose payload
      const targetedList = providerEpisodesReanime || providerEpisodesAnikoto || [];

      setAnimeData({
        episodeList: (targetedList || []).map((ep: any) => ({
          episodeId: ep.number,
          eps: ep.number,
